"""
ai_core.py — Unified AML AI Core Module
========================================
Consolidates Machine Learning ensemble model and statistical behavioral profiling.
...
Components:
- RandomForest + IsolationForest ensemble for typology-based risk (supervised)
- Behavioral profiling for customer-specific anomaly detection (unsupervised)
- Z-score amount deviations, velocity spikes, counterparty network changes
- Cold-start handling with neutral baseline
"""
...
_BOOTSTRAP = [
    {"amount": 45, "hour": 10, "transaction_type": "deposit", "sender_account": "A", "receiver_account": "A",
     ...
     "structuring_indicators": 0, "layering_indicators": 0},
    ...
    {"amount": 9500, "hour": 14, "transaction_type": "deposit", "sender_account": "E", "receiver_account": "E",
     ...
     "rapid_transfer_count": 0, "structuring_indicators": 2, "layering_indicators": 0},
    {"amount": 9800, "hour": 15, "transaction_type": "deposit", "sender_account": "F", "receiver_account": "F",
     ...
     "structuring_indicators": 1, "layering_indicators": 0},
    ...
    {"amount": 5500, "hour": 2, "transaction_type": "transfer", "sender_account": "G", "receiver_account": "H",
     ...
     "structuring_indicators": 0, "layering_indicators": 3},
    {"amount": 15000, "hour": 3, "transaction_type": "transfer", "sender_account": "I", "receiver_account": "J",
     ...
     "rapid_transfer_count": 0, "structuring_indicators": 0, "layering_indicators": 1},
...
# ============================================================================
# Behavioral Profiling Data Structures
# ============================================================================
...
# ============================================================================
# Behavioral Profiler Class
# ============================================================================

class BehavioralProfiler:
    """Main class for building and using customer behavioral profiles."""
    
    def __init__(self, min_transactions_for_profile: int = 5):
    ...
    def _score_amount_anomaly(self, profile: CustomerBehavioralProfile, amount: float) -> float:
        """Score how anomalous the amount is using Z-score calculation (0-100)."""
    ...
    def _score_frequency_anomaly(self, profile: CustomerBehavioralProfile, timestamp: datetime) -> float:
        """Score how anomalous the transaction frequency is using rolling window velocity deviations (0-100)."""
        # Calculate velocity deviation based on historical baseline
        # Compare current rolling window frequency against historical average
        
        # If we have insufficient data, return neutral score
        if profile.total_transactions < 5:
            return 0
        
        # Calculate expected frequency based on historical patterns
        expected_hourly_rate = profile.avg_daily_tx_count / 24
        expected_daily_rate = profile.avg_daily_tx_count
        
        # Get current hour's expected rate (adjusted for peak hours)
        current_hour = timestamp.hour
        if current_hour in profile.peak_hours:
            expected_hourly_rate *= 1.5  # Higher expected rate during peak hours
        else:
            expected_hourly_rate *= 0.5  # Lower expected rate during off-peak
        
        # Score based on deviation from expected rate
        # This is a simplified version - in production, would use actual rolling window counts
        # For now, return moderate score if transaction is during unusual hours
        if current_hour not in profile.peak_hours and profile.peak_hours:
            return 30
    ...
    def _score_recipient_anomaly(self, profile: CustomerBehavioralProfile, recipient: str) -> float:
        """Score how anomalous the recipient is using counterparty network change detection (0-100)."""
        ...
        # New recipient - assess based on counterparty network change
        # If customer has high diversity (uses many different recipients), new recipients are less anomalous
        # If customer has low diversity (few regular recipients), new recipients are more anomalous
