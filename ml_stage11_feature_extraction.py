"""
STAGE 11: Feature Extraction with new_recipient_ratio_7d Fix

This script extracts the approved 34 features from the Stage 11 dataset.
The only change from Stage 5 is the fix for new_recipient_ratio_7d calculation.
"""
...
print("=" * 80)
print("STAGE 11: 34-FEATURE EXTRACTION (WITH new_recipient_ratio_7d FIX)")
print("=" * 80)
print(f"Timestamp: {datetime.now().isoformat()}")
print()

# ============================================================================
# FEATURE DEFINITIONS (SAME AS STAGE 5)
# ============================================================================
...
32. new_recipient_ratio_7d (float) - Ratio of new recipients in last 7 days [FIXED]
...
class FeatureExtractor:
    """Extracts 34 features from Stage 11 dataset with temporal safety."""
    ...
    def compute_core_features(self, transaction: Dict, past_transactions: List[Dict]) -> Dict[str, float]:
        """Compute 14 core features from Stage 11 dataset."""
    ...
    def compute_behavioral_features(self, transaction: Dict, past_transactions: List[Dict]) -> Dict[str, float]:
        """Compute 13 new behavioral features with FIXED new_recipient_ratio_7d."""
        current_amount = float(transaction["amount"])
        current_timestamp = self.parse_timestamp(transaction["timestamp"])
        sender_account = transaction["sender_account"]
        ...
        # FIXED: New recipient ratio in 7d
        # Definition: recipients in last 7 days that were NOT seen before the 7-day window
        # (i.e., not seen in transactions older than 7 days)
        cutoff_7d = current_timestamp - timedelta(days=7)
        historical_recipients_before_7d = set(
            tx["receiver_account"] for tx in past_transactions
            if self.parse_timestamp(tx["timestamp"]) < cutoff_7d
        )
        new_recipients = sum(
            1 for tx in tx_7d
            if tx["receiver_account"] not in historical_recipients_before_7d
        )
...
def load_stage11_dataset(csv_path: str) -> List[Dict]:
    """Load Stage 11 dataset from CSV."""
    transactions = []
...
def main():
    ...
    transactions = load_stage11_dataset("ml_stage11_dataset.csv")
    ...
    export_features(feature_vectors, feature_names, "ml_stage11_features.csv")
    print("Features exported to ml_stage11_features.csv")
    print()
    
    print("=" * 80)
    print("FEATURE EXTRACTION COMPLETE (WITH new_recipient_ratio_7d FIX)")
    print("=" * 80)
