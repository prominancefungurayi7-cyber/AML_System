...
def test_structuring_and_high_risk_geography_are_detected():
    ...
    results = assess_rules(conn, amount=9_500, tx_type="transfer", sender="A", receiver="B", timestamp=now.isoformat(), destination_country="IR")
...
def test_velocity_fan_out_and_smurfing_require_real_prior_sequences():
    ...
    results = assess_rules(conn, amount=200, tx_type="transfer", sender="A", receiver="R5", timestamp=now.isoformat())
    assert {"R04", "R05"} <= _ids(results)

    conn = _db()
    for i in range(2):
        _insert(conn, 250, "deposit", "A", "A", now - timedelta(minutes=5 - i))
    results = assess_rules(conn, amount=250, tx_type="deposit", sender="A", receiver="A", timestamp=now.isoformat())
...
def test_off_hours_and_large_cash_are_detected_independently():
    ...
    results = assess_rules(conn, amount=12_000, tx_type="withdraw", sender="A", receiver="A", timestamp=timestamp.isoformat())
